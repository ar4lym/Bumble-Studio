...
export const firebaseConfig = {
  ...
  databaseURL: "https://bumble-studio-c4a23-default-rtdb.asia-southeast1.firebasedatabase.app",
...
const habitatAnimals = {
  "Park": ["Jellyfish", "Sunfish"],
  "Bedroom": ["Penguin", "Polarbear"],
  "Outdoor": ["Crocodile", "Frog"],
};
...
function loadBestTimesChart() {
  ...
  const habitats = ["Park", "Bedroom", "Mangroove", "Coral Reef"];
  const playersRef = ref(db, "players");
...
export function listenToLeaderboard(callback) {
  const sceneRef = ref(db, 'sceneEntries');
  onValue(sceneRef, (snapshot) => {
    ...
    for (let id in data) {
      let scene = data[id].sceneName;
      counts[scene] = (counts[scene] || 0) + 1;
    }

    // Send aggregated counts back to whoever calls this function
    callback(counts);
  });
}
